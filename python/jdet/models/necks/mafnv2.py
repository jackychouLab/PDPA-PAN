...
class MAFNv2(nn.Module):
    def __init__(self,
        ...
        for i in range(self.start_level, self.backbone_end_level):
            ...
            spp_g = nn.Sequential(*[SPP_G(out_channels)])
            self.SPP_Gs.append(spp_g)
    ...
    def execute(self, inputs):
        ...
        for i in range(len(laterals)):
            laterals[i] = self.SPP_Gs[i](laterals[i])
            laterals[i] = self.mid_convs[i](laterals[i])
