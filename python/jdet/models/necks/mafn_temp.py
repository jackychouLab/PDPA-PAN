        ...
        for i in range(self.start_level, self.backbone_end_level):
            ...
            if i != self.merge_level:
                if i < self.merge_level:
                    ...
                    self.modify1_convs.append(modify1_conv)
                    self.modify2_convs.append(modify2_conv)
            group_conv = nn.Conv2d(out_channels * 2, out_channels, self.merge_conv_size, stride=1, padding=1, groups=out_channels)
            self.group_convs.append(group_conv)

        self.merge_conv = nn.Conv2d(out_channels * (len(in_channels) - start_level), out_channels, self.merge_conv_size, stride=1, padding=1)
